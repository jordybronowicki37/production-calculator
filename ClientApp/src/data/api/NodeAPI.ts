...
async function nodeCreate(worksheetId: string, body: string): Promise<NodeDto> {
  const response = await fetch(`api/worksheet/${worksheetId}/node`, {
    method: "post",
...
export async function nodeEditPosition(
  ...
  const response = await fetch(`api/worksheet/${worksheetId}/node/${nodeId}/position`, {
    method: "put",
    headers: {"Content-Type": "application/json"},
...
export async function nodeEditProduct(
    ...
    nodeId: string,
    product: string): Promise<NodeDto> {
  const response = await fetch(`api/worksheet/${worksheetId}/node/${nodeId}/product`, {
    method: "put",
    headers: {"Content-Type": "application/json"},
...
export async function nodeEditRecipe(
  ...
  const response = await fetch(`api/worksheet/${worksheetId}/node/${nodeId}/recipe`, {
    method: "put",
    headers: {"Content-Type": "application/json"},
...
export async function nodeRemove(worksheetId: string, nodeId: string): Promise<void> {
  const response = await fetch(`api/worksheet/${worksheetId}/node/${nodeId}`, {method: "delete"});
  if (!response.ok) {
    const error = await response.text();
    throwErrorNotification(error);
    throw new Error(error);
